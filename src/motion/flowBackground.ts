// Original, reduced-density advection study inspired by epok.tech/work/tendrils/.
// Particles deposit momentum in a shared field; fading trails renew individually,
// so the background runs continuously without a whole-screen reset.
export function initFlowBackground(mode: "intro" | "curation"): () => void {
  ...
  const trailLength = mode === "curation" ? 96 : 48;
  ...
  const step = () => {
    ...
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < columns; x++) {
        ...
        let dy: number;
        if (mode === "intro") {
          const angle = -0.3 + 0.85 * Math.sin(u * 5.2 + v * 3.8 + 0.35 * Math.sin(phase))
            + 0.42 * Math.cos(v * 7 - u * 2.4 + 0.3 * Math.cos(phase));
          dx = Math.cos(angle);
          dy = Math.sin(angle);
        } else {
          // Long descending folds: an open current, with no vortex centres.
  ...
  const paint = () => {
    ...
    for (let band = 0; band < 6; band++) {
      ...
      context.strokeStyle = `rgba(100, 159, 126, ${(band + 1) * (mode === "curation" ? 0.025 : 0.032)})`;
      context.stroke();
    }
  };
  ...
  const resize = () => {
    ...
    for (let i = 0; i < (mode === "curation" ? 112 : 70); i++) step();
    paint();
  };

  const render = (now: number) => {
    frame = 0;
    if (disposed || document.hidden || reduced.matches || document.documentElement.classList.contains("is-index-open")) return;
  ...
  const sync = () => {
    ...
    if (!disposed && !document.hidden && !reduced.matches && !document.documentElement.classList.contains("is-index-open")) {
      frame = requestAnimationFrame(render);
    }
  };
  const indexObserver = new MutationObserver(sync);
